...
- 新增 `plain` 与 `glass` 主题。
- 主题文件的颜色字段新增 `"default"`，将该槽位交给终端，作背景时等同 SGR 49，作前景时等同 SGR 39。

### Changed

- 模型 API 默认请求改为携带 letcode 自己的 User-Agent（`letcode (<os> <arch>; <version>)`）。
- 主题选择器、伪装模式、思考显示、工具显示与推理强度的说明文字接入翻译。

### Fixed

- 修复后台子代理完成后主代理没有继续工作的问题。
- 修复中断一个正在跑的回合后，会话可能因工具调用批次未闭合而无法继续的问题。
