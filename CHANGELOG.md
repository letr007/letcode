...
- 实验性审批后端：`[agents.reviewer]` 指向声明 `reviewer = "jev"` 的 provider 时，`auto` 模式改由 Typesafe Jev 判定（每次一个 `choice` 问题、返回概率），其中转地址、凭据和模型都取自该 provider；未标记时仍用 chat 审查专家。

### Changed

- `auto` 权限模式下审查专家的判定从放行/拒绝两档扩展为三档：可以直接执行、交回请求方补充说明、或拒绝执行；补充说明后仍不明确才弹出与 `default` 模式相同的人工审批，而升级到用户的那一轮需要交互式前端，one-shot 等无交互入口无法回答。
- 命令不再有只读白名单：`shell__exec` 一律交给审批（`auto` 下由 reviewer 判定），只按读/写分流，因为命令文本无法证明无副作用。
- 人工审批提示改为左右方向键在横排选项间切换高亮、Enter 确认，上下键恢复为滚动对话；审批待处理时终端标题以 `!` 提醒，与提问待处理的 `?` 一致。

### Removed

- 移除只读/低风险命令白名单（`rg`、`ls`、`git status`、`cargo check` 等前缀），以及只服务于它的命令风险分类。
...
- 新增会话归档：超过 `[global.session_archive] older_than_days`（默认 7 天）未活动的会话族会在后台压缩移出磁盘，会话列表照常显示，恢复时自动还原。
- 新建会话继承当前会话的模型路由、专家路由、推理强度与请求兼容模式。
- `/agents` 专家列表逐项列出已选中的模型路由，选中项在被清除前替代配置默认。
- 专家委派未指定模型时默认使用该专家已选中允许路由中的第一条。

### Changed

- 终端窄于 84 列时不再渲染会话面板，加宽后恢复。
- 移除一处历史遗留的关键词启发式实现。

### Fixed

- 修复窄终端下会话面板透出底层文字、区域内点击和拖拽仍命中底层 transcript 的问题。
- 修复 Markdown 表格单元格超宽被截断、CJK 列被挤压后渲染为空的问题。
...
- 修复不支持图片输入的模型通过 `fs__read` 读取图片，或接收用户粘贴图片后，在后续请求/会话恢复时因 `unsupported_request_field` 失败的问题；图片会被拒绝或转换为文本占位信息。
