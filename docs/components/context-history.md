...
Historian 每次只读取尚未整理前缀的一段，上限为 Historian 路由可用输入预算的四分之一，介于 2 万与 5 万 token；其余部分留给后续请求边界。按工作目标生成历史段，每段一次生成三种表述：
...
模型必须已经在对应 provider 中配置。Historian 不要求路由声明 `structured_output`：发布契约由任务 prompt 与宿主解析器承担，路由是否能在解码层约束 JSON 不影响它能否承担整理工作。响应按行读取，每行一个历史段；第一行无法解析、或切割位置不前进时，该行及其后的内容被丢弃，已解析的部分作为前缀发布，剩余消息留给后续整理。契约被破坏时，同一次运行会对同一段输入用修复提示重问一次。Historian 使用统一 ModelRuntime 的无工具 one-shot 请求，静态任务约束进入协议原生高权威字段，不进入主 Agent 的工具循环，不递归委派。处理范围按 Historian 路由的请求预算与分片上限预检；未被发布推进的同一段前缀在连续失败达上限后进入冷却窗口，期间不再重发，窗口过后重新尝试。
