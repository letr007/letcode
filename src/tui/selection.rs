...
mod tests {
    ...
    #[test]
    fn separate_timeline_items_insert_one_visible_newline() {
        use crate::tui::{
            components::transcript::TranscriptRenderCacheEntry,
            transcript_render::{Break, Document, Line, SourceRange, Span},
        };
        ...
        let mut state = TuiState::default();
        let mut first = Document::<ratatui::style::Style>::default();
        let first_block = first.add_source("first");
        first.push_line(
            Line {
                spans: vec![Span::source(
                    "first",
                    ratatui::style::Style::default(),
                    SourceRange::new(first_block, 0, 5),
                )],
            },
            Break::End,
        );
        let mut second = Document::<ratatui::style::Style>::default();
        let second_block = second.add_source("second");
        second.push_line(
            Line {
                spans: vec![Span::source(
                    "second",
                    ratatui::style::Style::default(),
                    SourceRange::new(second_block, 0, 6),
                )],
            },
            Break::End,
        );
    ...
    #[test]
    fn combining_and_zwj_selection_expand_to_whole_graphemes() {
        use crate::tui::{
            components::transcript::TranscriptRenderCacheEntry,
            transcript_render::{Break, Document, Line, SourceRange, Span},
        };
        ...
        assert_eq!(extract_selected_text(&state), "e\u{301}👩‍💻");
    }
    ...
    #[test]
    fn same_point_selection_includes_the_hit_grapheme() {
        use crate::tui::{
            components::transcript::TranscriptRenderCacheEntry,
            transcript_render::{Break, Document, Line, SourceRange, Span},
        };
        ...
        let mut document = Document::<ratatui::style::Style>::default();
        let block = document.add_source("a你");
        document.push_line(
            Line {
                spans: vec![Span::source(
                    "a你",
                    ratatui::style::Style::default(),
                    SourceRange::new(block, 0, 2),
                )],
            },
            Break::End,
        );
        ...
        assert_eq!(extract_selected_text(&state), "你");
    }

    #[test]
    fn copy_join_and_chrome_boundaries_preserve_semantic_text() {
        use crate::tui::{
            components::transcript::TranscriptRenderCacheEntry,
            transcript_render::{Break, CopyJoin, Document, Line, SourceRange, Span},
        };

        let mut document = Document::<ratatui::style::Style>::default();
        let first = document.add_source("summary");
        let second = document.add_source("reason");
        document.push_line(
            Line {
                spans: vec![
                    Span::source(
        ...
        document.push_line(
            Line {
                spans: vec![Span::decoration("chrome", ratatui::style::Style::default())],
            },
            Break::BlockBreak,
        );
        let third = document.add_source("next");
        document.push_line(
            Line {
                spans: vec![Span::source(
                    "next",
                    ratatui::style::Style::default(),
                    SourceRange::new(third, 0, 4),
                )],
            },
            Break::End,
        );
        ...
        state.text_selection = Some(TextSelection {
            start: SelectionAnchor {
                ...
                item_index: 0,
                rendered_line_offset: 2,
                char_offset: 3,
            },
        });

        assert_eq!(extract_selected_text(&state), "summary reason\nnext");
    }
}
