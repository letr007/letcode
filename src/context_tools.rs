...
impl ToolHandler for ContextListTool {
    fn name(&self) -> &str { tool_names::TOOL_CONTEXT_LIST }
    fn description(&self) -> &str { "List available context nodes, blocks, summaries, and folded outputs." }
    fn permission_class(&self) -> ToolPermissionClass { ToolPermissionClass::Read }
    ...
    async fn execute_with_context(&self, args: Value, context: ToolExecutionContext) -> Result<Value> {
        ...
        let include_removed = args.get("include_removed").and_then(Value::as_bool).unwrap_or(false);
        ...
        let blocks = sorted_blocks(&projection)
            .into_iter()
            .filter(|(id, block)| block_visible_for_listing(&projection, id.as_str(), block, include_archived, include_removed))
            .take(limit)
            .map(|(id, block)| block_ref_json(&projection, id.as_str(), block))
            .collect::<Vec<_>>();

        let summaries = projection.summary_artifacts.iter().take(limit).map(|artifact| {
            json!({
                "ref_type":"summary",
        ...
        let folded_outputs = sorted_blocks(&projection)
            .into_iter()
            .filter(|(id, block)| block.folded_output_id.is_some() && block_visible_for_listing(&projection, id.as_str(), block, include_archived, include_removed))
        ...
        Ok(json!({"ok":true,"nodes":nodes,"blocks":blocks,"summaries":summaries,"folded_outputs":folded_outputs}))
    }
}

#[async_trait]
impl ToolHandler for ContextSearchTool {
    fn name(&self) -> &str { tool_names::TOOL_CONTEXT_SEARCH }
    fn description(&self) -> &str { "Search context nodes, blocks, summaries, and folded outputs." }
    fn permission_class(&self) -> ToolPermissionClass { ToolPermissionClass::Read }
    fn parameters(&self) -> Value {
        json!({
            "type":"object",
    ...
    async fn execute_with_context(&self, args: Value, context: ToolExecutionContext) -> Result<Value> {
        ...
        for node in tree.nodes() {
            let haystack = format!(
                ...
                node.block_ref.as_ref().map(|block_ref| block_ref.block_id.as_str()).unwrap_or(""),
                node.source_ref.as_ref().map(format_source_ref).unwrap_or_default()
            );
        ...
        for (id, block) in sorted_blocks(&projection) {
            if !block_visible_for_listing(&projection, id.as_str(), block, include_archived, include_removed) {
                continue;
            }
            let haystack = format!(
                "{} {} {} {} {} {}",
        ...
        for artifact in &projection.summary_artifacts {
            ...
            let haystack = format!(
                ...
                artifact.source_start_sequence.map(|value| value.to_string()).unwrap_or_default(),
                artifact.source_end_sequence.map(|value| value.to_string()).unwrap_or_default()
            );
            if haystack.to_ascii_lowercase().contains(&query_lower) {
        ...
        for metadata in projection.folded_outputs.values() {
            if matches.len() >= limit { break; }
            let Some((block_id, block)) = folded_block_for_output(&projection, &metadata.output_id) else { continue; };
            if !block_visible_for_listing(&projection, block_id, block, include_archived, include_removed) {
                continue;
            }
            let haystack = format!(
                "{} {} {} {} {} {} {} {} {}",
                ...
                metadata.byte_count,
                metadata.line_count,
                metadata.source_start_sequence.map(|value| value.to_string()).unwrap_or_default(),
                metadata.source_end_sequence.map(|value| value.to_string()).unwrap_or_default()
            );
            if haystack.to_ascii_lowercase().contains(&query_lower) {
                matches.push(folded_output_match_json(&projection, block_id, block, metadata));
            }
        }
...
impl ToolHandler for ContextOpenTool {
    fn name(&self) -> &str { tool_names::TOOL_CONTEXT_OPEN }
    fn description(&self) -> &str { "Open a context node, block, summary, or folded output by stable reference." }
    fn permission_class(&self) -> ToolPermissionClass { ToolPermissionClass::Read }
    fn parameters(&self) -> Value {
        json!({
            "type":"object",
    ...
    async fn execute_with_context(&self, args: Value, context: ToolExecutionContext) -> Result<Value> {
        ...
        match ref_type.as_str() {
            ...
            "block" => {
                let block = projection.blocks.iter().find(|(id, _)| id.as_str() == ref_id)
                    .map(|(_, block)| block)
            ...
            "summary" => {
                let artifact = projection.open_summary_artifact(&ref_id)
            ...
            "folded_output" => {
                let metadata = projection.folded_outputs.get(&ref_id)
                ...
                let open = projection.open_folded_output(&ref_id, max_bytes)
                    .ok_or_else(|| anyhow!("unknown folded output '{ref_id}'"))?;
                Ok(json!({
                    "ok":true,
...
impl ToolHandler for ContextPinTool {
    fn name(&self) -> &str { tool_names::TOOL_CONTEXT_PIN }
    fn description(&self) -> &str { "Pin a context block for future prompt inclusion." }
    fn permission_class(&self) -> ToolPermissionClass { ToolPermissionClass::Preview }
    fn parameters(&self) -> Value { block_mutation_schema() }
    async fn execute(&self, _args: Value) -> Result<Value> { bail!("context view projection unavailable") }
    async fn execute_with_context(&self, args: Value, context: ToolExecutionContext) -> Result<Value> {
        validate_block_operation(&context, &required_trimmed_string(&args, "block_id", MAX_ID_CHARS)?, "pin")
    }
}

#[async_trait]
impl ToolHandler for ContextArchiveTool {
    fn name(&self) -> &str { tool_names::TOOL_CONTEXT_ARCHIVE }
    fn description(&self) -> &str { "Archive a context block from the default view." }
    fn permission_class(&self) -> ToolPermissionClass { ToolPermissionClass::Preview }
    fn parameters(&self) -> Value { block_mutation_schema() }
    async fn execute(&self, _args: Value) -> Result<Value> { bail!("context view projection unavailable") }
    async fn execute_with_context(&self, args: Value, context: ToolExecutionContext) -> Result<Value> {
        validate_block_operation(&context, &required_trimmed_string(&args, "block_id", MAX_ID_CHARS)?, "archive")
    }
}

#[async_trait]
impl ToolHandler for ContextRemoveTool {
    fn name(&self) -> &str { tool_names::TOOL_CONTEXT_REMOVE }
    fn description(&self) -> &str { "Remove a context block from the default view." }
    fn permission_class(&self) -> ToolPermissionClass { ToolPermissionClass::Preview }
    fn parameters(&self) -> Value { block_mutation_schema() }
    async fn execute(&self, _args: Value) -> Result<Value> { bail!("context view projection unavailable") }
    async fn execute_with_context(&self, args: Value, context: ToolExecutionContext) -> Result<Value> {
        validate_block_operation(&context, &required_trimmed_string(&args, "block_id", MAX_ID_CHARS)?, "remove_from_view")
    }
}

#[async_trait]
impl ToolHandler for ContextSummarizeTool {
    fn name(&self) -> &str { tool_names::TOOL_CONTEXT_SUMMARIZE }
    fn description(&self) -> &str { "Validate a proposed context summary artifact and return metadata to record." }
    fn permission_class(&self) -> ToolPermissionClass { ToolPermissionClass::Preview }
    fn parameters(&self) -> Value {
        json!({
            "type":"object",
    ...
    async fn execute_with_context(&self, args: Value, context: ToolExecutionContext) -> Result<Value> {
        ...
        ensure!(tree.node(&node_ref).is_some(), "unknown context node '{node_id}'");
        let artifact_id = required_trimmed_string(&args, "artifact_id", MAX_ID_CHARS)?;
        ensure!(
            projection.summary_artifacts.iter().all(|artifact| artifact.artifact_id != artifact_id),
        ...
        if let Some(block_id) = &source_block_id {
            ensure!(projection.blocks.keys().any(|id| id.as_str() == block_id), "unknown context block '{block_id}'");
        }
        ...
        if let Some(source_node_id) = &source_node_id {
            ...
            ensure!(tree.node(&source_node_ref).is_some(), "unknown context node '{source_node_id}'");
        }
        let artifact_kind = optional_trimmed_string(&args, "artifact_kind", MAX_ARTIFACT_KIND_CHARS)?.unwrap_or_else(|| "summary".into());
        ...
        let source_end_sequence = args.get("source_end_sequence").and_then(Value::as_u64);
            ...
            source_block_id.is_some() || source_node_id.is_some() || source_start_sequence.is_some(),
        ...
        if let (Some(start), Some(end)) = (source_start_sequence, source_end_sequence) {
            ensure!(start <= end, "source_start_sequence must be <= source_end_sequence");
        }
        Ok(json!({
            "ok":true,
            "summary_metadata":{
...
fn validate_block_operation(context: &ToolExecutionContext, block_id: &str, operation: &str) -> Result<Value> {
    let projection = require_projection(context)?;
    let mut state = projection.view_state.clone();
    let op = match operation {
        "pin" => crate::context_view::ContextViewOperation::Pin { block_id: crate::context_view::ContextBlockId::new(block_id.to_string())? },
        "archive" => crate::context_view::ContextViewOperation::Archive { block_id: crate::context_view::ContextBlockId::new(block_id.to_string())? },
        "remove_from_view" => crate::context_view::ContextViewOperation::RemoveFromView { block_id: crate::context_view::ContextBlockId::new(block_id.to_string())? },
        other => bail!("unsupported context operation '{other}'"),
    };
    state.apply(&projection.blocks, &op).map_err(|error| anyhow!(error.to_string()))?;
    Ok(json!({
        "ok":true,
...
fn require_projection(context: &ToolExecutionContext) -> Result<Arc<ContextViewProjection>> {
    context.context_view.clone().ok_or_else(|| anyhow!("context view projection unavailable"))
}

fn require_context_tree(context: &ToolExecutionContext) -> Result<Arc<ContextTreeState>> {
    context.context_tree.clone().ok_or_else(|| anyhow!("context tree snapshot unavailable"))
}

fn parse_limit(args: &Value, default: usize) -> Result<usize> {
    let limit = match args.get("limit") {
        Some(Value::Null) | None => default,
        Some(value) => usize::try_from(value.as_u64().ok_or_else(|| anyhow!("limit must be integer or null"))?)
            .map_err(|_| anyhow!("limit is too large"))?,
    };
    ensure!(limit > 0 && limit <= MAX_LIST_LIMIT, "limit must be between 1 and {MAX_LIST_LIMIT}");
    Ok(limit)
}
...
fn parse_max_bytes(args: &Value) -> Result<usize> {
    let max_bytes = match args.get("max_bytes") {
        ...
        Some(value) => usize::try_from(value.as_u64().ok_or_else(|| anyhow!("max_bytes must be integer or null"))?)
            .map_err(|_| anyhow!("max_bytes is too large"))?,
    };
    ensure!(max_bytes > 0 && max_bytes <= MAX_OPEN_MAX_BYTES, "max_bytes must be between 1 and {MAX_OPEN_MAX_BYTES}");
    Ok(max_bytes)
}

fn required_trimmed_string(args: &Value, field: &str, max_chars: usize) -> Result<String> {
    let value = args.get(field).and_then(Value::as_str).ok_or_else(|| anyhow!("field '{field}' must be a string"))?;
    let trimmed = value.trim();
    ensure!(!trimmed.is_empty(), "field '{field}' must not be empty");
    ensure!(trimmed.chars().count() <= max_chars, "field '{field}' exceeds {max_chars} characters");
...
fn optional_trimmed_string(args: &Value, field: &str, max_chars: usize) -> Result<Option<String>> {
    match args.get(field) {
        ...
        Some(Value::String(value)) => {
            let trimmed = value.trim();
            ensure!(!trimmed.is_empty(), "field '{field}' must not be empty when provided");
            ensure!(trimmed.chars().count() <= max_chars, "field '{field}' exceeds {max_chars} characters");
...
fn sorted_blocks(projection: &ContextViewProjection) -> Vec<(&crate::context_view::ContextBlockId, &ContextBlock)> {
    let mut blocks = projection.blocks.iter().collect::<Vec<_>>();
    blocks.sort_by(|(left_id, left), (right_id, right)| {
        left.source_start_sequence.or(left.available_sequence).unwrap_or(u64::MAX)
...
fn block_visible_for_listing(
    ...
    if block.is_protected() { return true; }
...
fn block_status_string(projection: &ContextViewProjection, block_id: &str) -> String {
    match projection.blocks.keys().find(|id| id.as_str() == block_id)
...
fn block_ref_json(projection: &ContextViewProjection, block_id: &str, block: &ContextBlock) -> Value {
    json!({
        "ref_type":"block",
        "ref_id":block_id,
        "title":block.title,
...
fn open_node_json(
    ...
    let child_node_ids = tree.nodes()
        .filter(|candidate| candidate.parent_node_id.as_ref() == Some(&node.node_id))
        .map(|candidate| candidate.node_id.as_str().to_string())
        .collect::<Vec<_>>();
    let summaries = projection.summary_artifacts.iter()
        .filter(|artifact| artifact.node_id == node.node_id.as_str() || artifact.source_node_id.as_deref() == Some(node.node_id.as_str()))
        .map(|artifact| {
            json!({
        ...
        .collect::<Vec<_>>();
    let referenced_block = node.block_ref.as_ref()
        .and_then(|block_ref| open_node_referenced_block_json(projection, &block_ref.block_id, max_bytes));
...
fn open_node_referenced_block_json(
    ...
    let block = projection.blocks.iter().find(|(id, _)| id.as_str() == block_id)
        .map(|(_, block)| block)?;
...
fn ensure_block_openable(projection: &ContextViewProjection, block_id: &str) -> Result<String> {
    let status = block_status_string(projection, block_id);
    ensure!(status != "removed_from_view", "cannot open removed context block '{block_id}'");
    Ok(status)
}
...
fn format_block_source(source: &ContextBlockSource) -> String {
    match source {
        ContextBlockSource::TranscriptSpan { start_sequence, end_sequence } => format!("transcript:{start_sequence}..{end_sequence}"),
...
fn next_summary_version(projection: &ContextViewProjection, node_id: &str, artifact_kind: &str) -> u32 {
    projection.summary_artifacts.iter()
    ...
    match args.get("version") {
        ...
        Some(value) => {
            ...
            let version = u32::try_from(raw)
                .map_err(|_| anyhow!("version must be <= {}", u32::MAX))?;
...
fn truncate(text: &str, max_chars: usize) -> String {
    ...
    if text.chars().count() > max_chars { truncated.push('…'); }
    truncated
}
...
mod tests {
    ...
    fn record(sequence: u64, event: TranscriptEvent) -> TranscriptRecord {
        TranscriptRecord { session_id: "s".into(), sequence, timestamp_ms: 0, context_branch_id: None, event }
    }

    fn base_records() -> Vec<TranscriptRecord> {
        vec![
            record(1, TranscriptEvent::UserMessage { content: UserMessageContent::from("Do not drop this requirement") }),
            record(2, TranscriptEvent::AssistantMessage { content: "visible note".into() }),
            record(3, TranscriptEvent::ContextViewOperationMetadata { operation: "pin".into(), node_id: None, block_id: Some("block-seq-2-note".into()), detail: None }),
            record(4, TranscriptEvent::ToolCallStarted { call_id: "call-1".into(), name: "shell__exec".into(), args: json!({"command":"cargo test"}) }),
            record(5, TranscriptEvent::ToolCallFinished { call_id: "call-1".into(), name: "shell__exec".into(), ok: true, output: crate::tool::ToolResult::ok("shell__exec", json!({"status":0,"stdout":"x".repeat(5000),"stdout_truncated":false,"stderr":"","stderr_truncated":false})) }),
        ]
    }

    fn tree_records() -> Vec<TranscriptRecord> {
        vec![
            record(6, TranscriptEvent::ContextNodeCreated {
                node_id: "node-a".into(),
                parent_node_id: Some("root".into()),
                label: Some("Build node".into()),
                purpose: Some("Track build context".into()),
                block_ref: Some(crate::context_tree::ContextBlockRef { block_id: "block-seq-2-note".into() }),
                source_ref: Some(crate::context_tree::ContextSourceRef { source_kind: "summary".into(), source_id: Some("sum-node-a".into()) }),
            }),
            record(7, TranscriptEvent::ContextNodeLifecycle { node_id: "node-a".into(), status: crate::context_tree::ContextNodeStatus::Inactive }),
            record(8, TranscriptEvent::ContextSummaryArtifactMetadata {
                node_id: "node-a".into(),
    ...
    fn projection_with_summary_artifact() -> Arc<ContextViewProjection> {
        Arc::new(project_context_view(&[
            record(1, TranscriptEvent::AssistantMessage { content: "visible note".into() }),
            record(2, TranscriptEvent::ContextSummaryArtifactMetadata {
                node_id: "node-a".into(),
    ...
    fn context(projection: Option<Arc<ContextViewProjection>>, tree: Option<Arc<ContextTreeState>>) -> ToolExecutionContext {
        ToolExecutionContext { allow_outside_workspace: false, context_view: projection, context_tree: tree }
    }

    #[tokio::test]
    async fn context_tools_fail_fast_when_projection_is_missing() {
        let registry = ToolRegistry::default_tools();
        let output = registry.call_with_context(tool_names::TOOL_CONTEXT_LIST, json!({"include_archived":false,"include_removed":false,"limit":null}), context(None, None)).await;
        assert!(!output.ok);
        assert!(output.error.as_ref().is_some_and(|error| error.message.contains("projection unavailable")));
    }
    ...
    async fn context_list_search_and_open_work_from_snapshot() {
        ...
        let tree = tree();
        let listed = registry.call_with_context(tool_names::TOOL_CONTEXT_LIST, json!({"include_archived":false,"include_removed":false,"limit":null}), context(Some(projection.clone()), Some(tree.clone()))).await;
        assert!(listed.ok, "{listed:?}");
        let nodes = listed.data.as_ref().and_then(|d| d.get("nodes")).and_then(Value::as_array).expect("nodes");
        assert!(nodes.iter().any(|node| node["ref_id"] == "node-a"));
        let blocks = listed.data.as_ref().and_then(|d| d.get("blocks")).and_then(Value::as_array).expect("blocks");
        assert!(blocks.iter().any(|block| block["ref_id"] == "block-seq-2-note"));
        ...
        assert!(searched.data.as_ref().and_then(|d| d.get("matches")).and_then(Value::as_array).is_some_and(|items| !items.is_empty()));

        let opened = registry.call_with_context(tool_names::TOOL_CONTEXT_OPEN, json!({"ref_type":"block","ref_id":"block-seq-2-note","max_bytes":null}), context(Some(projection), Some(tree))).await;
        assert!(opened.ok, "{opened:?}");
        assert_eq!(opened.data.as_ref().and_then(|d| d.get("operation_metadata")).and_then(|d| d.get("operation")), Some(&json!("open_detail")));
    }
    ...
    async fn context_mutations_validate_and_fail_atomically() {
        let registry = ToolRegistry::default_tools();
        let projection = projection_with_tree_data();
        let pinned = registry.call_with_context(tool_names::TOOL_CONTEXT_PIN, json!({"block_id":"block-seq-2-note"}), context(Some(projection.clone()), None)).await;
        assert!(pinned.ok, "{pinned:?}");
        assert_eq!(pinned.data.as_ref().and_then(|d| d.get("pending_recording")), Some(&json!(true)));

        let protected_archive = registry.call_with_context(tool_names::TOOL_CONTEXT_ARCHIVE, json!({"block_id":"block-seq-1-user-requirement"}), context(Some(projection.clone()), None)).await;
        assert!(!protected_archive.ok);
        assert!(protected_archive.error.as_ref().is_some_and(|error| error.message.contains("protected context block")));

        let still_listed = registry.call_with_context(tool_names::TOOL_CONTEXT_LIST, json!({"include_archived":false,"include_removed":false,"limit":null}), context(Some(projection), Some(tree()))).await;
        assert!(still_listed.ok);
        assert!(still_listed.data.as_ref().and_then(|d| d.get("blocks")).and_then(Value::as_array).is_some_and(|blocks| blocks.iter().any(|block| block["ref_id"] == "block-seq-1-user-requirement")));
    }
    ...
    async fn context_summarize_validates_and_returns_metadata() {
        let registry = ToolRegistry::default_tools();
        let projection = projection_with_tree_data();
        let output = registry.call_with_context(tool_names::TOOL_CONTEXT_SUMMARIZE, json!({
            "node_id":"node-a",
            "artifact_id":"sum-1",
            ...
            "artifact_kind":null,
            "version":null
        }), context(Some(projection), Some(tree()))).await;
        assert!(output.ok, "{output:?}");
        assert_eq!(output.data.as_ref().and_then(|d| d.get("summary_metadata")).and_then(|d| d.get("version")), Some(&json!(2)));
    }
    ...
    async fn context_open_rejects_removed_folded_output() {
        ...
        let output = registry.call_with_context(tool_names::TOOL_CONTEXT_OPEN, json!({
            "ref_type":"folded_output",
    ...
    async fn context_summarize_rejects_duplicate_artifact_id() {
        ...
        assert!(!output.ok);
        assert!(output.error.as_ref().is_some_and(|error| error.message.contains("duplicate artifact_id 'sum-1'")));
    }
    ...
    async fn context_summarize_rejects_oversized_version() {
        ...
        assert!(!output.ok);
        assert!(output.error.as_ref().is_some_and(|error| error.message.contains(&format!("version must be <= {}", u32::MAX))));
    }
    ...
    async fn context_summarize_rejects_missing_traceable_source() {
        ...
        assert!(!output.ok);
        assert!(output.error.as_ref().is_some_and(|error| error.message.contains("requires at least one traceable source")));
    }
    ...
    async fn context_summarize_rejects_unknown_source_node_id() {
        ...
        assert!(!output.ok);
        assert!(output.error.as_ref().is_some_and(|error| error.message.contains("unknown context node 'node-missing'")));
    }
    ...
    async fn context_search_finds_node() {
        let registry = ToolRegistry::default_tools();
        let projection = projection_with_tree_data();
        let output = registry.call_with_context(tool_names::TOOL_CONTEXT_SEARCH, json!({
            "query":"Build node",
        ...
        assert!(output.ok, "{output:?}");
        assert!(output.data.as_ref().and_then(|d| d.get("matches")).and_then(Value::as_array).is_some_and(|items| items.iter().any(|item| item["ref_type"] == "node" && item["ref_id"] == "node-a")));
    }
    ...
    async fn context_search_returns_rich_block_and_folded_payloads() {
        let registry = ToolRegistry::default_tools();
        let projection = projection_with_tree_data();
        let block_output = registry.call_with_context(tool_names::TOOL_CONTEXT_SEARCH, json!({
            "query":"visible note",
            "include_archived":false,
            "include_removed":false,
            "limit":null
        ...
        let block_matches = block_output.data.as_ref().and_then(|d| d.get("matches")).and_then(Value::as_array).expect("block matches");
        let block = block_matches.iter()
        ...
        let folded_output = registry.call_with_context(tool_names::TOOL_CONTEXT_SEARCH, json!({
            "query":"cargo test",
            "include_archived":false,
            "include_removed":false,
            "limit":null
        }), context(Some(projection_with_tree_data()), Some(tree()))).await;

        assert!(folded_output.ok, "{folded_output:?}");
        let folded_matches = folded_output.data.as_ref().and_then(|d| d.get("matches")).and_then(Value::as_array).expect("folded matches");
        let folded = folded_matches.iter().find(|item| item["ref_type"] == "folded_output").expect("folded match");
    ...
    async fn context_search_finds_summary_source_metadata() {
        ...
        assert!(output.ok, "{output:?}");
        assert!(output.data.as_ref().and_then(|d| d.get("matches")).and_then(Value::as_array).is_some_and(|items| items.iter().any(|item| {
    ...
    async fn context_open_node_by_id_returns_snapshot_metadata() {
        ...
        assert!(data.get("related_summaries").and_then(Value::as_array).is_some_and(|items| items.iter().any(|item| item["artifact_id"] == "sum-node-a")));
        assert_eq!(data.get("referenced_block").and_then(|block| block.get("ref_id")), Some(&json!("block-seq-2-note")));
    }
}
