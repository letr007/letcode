...
mod tests {
    ...
    fn parse_model_route_accepts_provider_qualified_and_legacy_model_ids() {
        ...
        assert_eq!(
            cli::parse_model_route(&config, "expert/shared"),
            config::ModelRoute::new("expert", "shared")
        );
        assert_eq!(
            cli::parse_model_route(&config, "shared"),
            config::ModelRoute::new("primary", "shared")
        );
        assert_eq!(
            cli::parse_model_route(&config, "vendor/model"),
    ...
    fn one_shot_error_record_preserves_anyhow_chain() {
        ...
        cli::record_one_shot_error(&recorder, &error).expect("error should be recorded");
